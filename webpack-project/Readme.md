...
2. 第二个应用：预获取(prefetch)和预加载(preload)

- 使用 prefetch（在动态导入时添加一个魔法注释 webpackPrefetch: true）的意义在于：告诉 webpack 执行预获取，这会生成 <link rel="prefetch" as="script" href="math.bundle.js"> 并追加到页面的 head 标签。当首页面的内容都加载完毕，在网络空闲的时候，再去加载打包好的 math.bundle.js
- 使用 preload（在动态导入时添加一个魔法注释 webpackPreload: true）和懒加载的效果差不多
